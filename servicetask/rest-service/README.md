...
This quickstart demonstrates how to use the built-in REST HTTP connector for invoking REST services in Orqueio Platform. The example is *classless*, relying entirely on scripting and expression language. It makes use of the orqueio Connect and the orqueio Spin extensions.
...
The important part of this example is the service task *Get holiday*. It uses the HTTP connector provided by orqueio Connect and processes the returned JSON using orqueio Spin. For reference, have a look at the complete [BPMN XML][3].

### Configuring Connect and Spin

Since Connect and Spin are optional extensions to the orqueio platform, they have to be added as dependencies in [pom.xml][4]:
...
Note the `dependencyManagement` entry. It declares the orqueio bill of material (BOM) that provides managed dependencies for all orqueio artifacts. These are referenced in the *dependencies* section. The BOM ensures that the version of the dependencies are consistent with each other.

Next, the process engine plugins provided by Spin and Connect are registered with the engine in [orqueio.cfg.xml][5]:

```xml
...
<beans ...>

  <bean id="processEngineConfiguration" class="io.orqueio.bpm.engine.impl.cfg.StandaloneInMemProcessEngineConfiguration">

    ...
    <!-- activates the orqueio Connect process engine plugins -->
...
1. Invoke a REST web service using the HTTP connector provided by orqueio Connect.
2. Extract a property indicating whether a day is a holiday from the service's JSON response using Javascript and orqueio Spin. This variable is used on the follow-up exclusive gateway.
...
The *outputParameter* element maps the response obtained by the connector to a variable `isHoliday`. The mapping makes use of orqueio Spin to extract an element from the returned JSON and is implemented in a [Javascript file][6].

[1]: src/main/resources/invokeRestService.png
[2]: src/test/java/io/orqueio/bpm/example/servicetask/rest/ServiceTaskRestTest.java
[3]: src/main/resources/invokeRestService.bpmn
[4]: pom.xml#L20-L84
[5]: src/test/resources/orqueio.cfg.xml
