 ...
 */
package io.orqueio.bpm.example.modelapi;
...
import io.orqueio.bpm.model.bpmn.instance.ExclusiveGateway;
...
public class ParseBpmnTest {
  ...
  public void readAttributes() {
    ...
    // you can also access orqueio extension attributes
    assertThat(serviceTask.getOrqueioExpression()).isEqualTo("${execution.setVariable('foo', 'bar')}");
    ...
    assertThat(userTaskA.getOrqueioCandidateGroupsList()).containsExactly("management", "accounting");

    UserTask userTaskB = modelInstance.getModelElementById("userTaskB");
    assertThat(userTaskB.getOrqueioAssignee()).isEqualTo("demo");
  ...
  public void readExtensionElements() {
    ...
    for (OrqueioFormField formField : formData.getOrqueioFormFields()) {
      ...
      assertThat(formField.getOrqueioType()).isIn("string", "long");
      assertThat(formField.getOrqueioDefaultValue()).isIn("test", "0");
    }

    // the end event contains an execution listener
    EndEvent endEvent = modelInstance.getModelElementById("endEvent");
    OrqueioExecutionListener executionListener = endEvent.getExtensionElements().getElementsQuery().filterByType(OrqueioExecutionListener.class).singleResult();
    assertThat(executionListener.getOrqueioEvent()).isEqualTo("start");
    assertThat(executionListener.getOrqueioExpression()).isEqualTo("${execution.setVariable('finished', true)}");
  }
